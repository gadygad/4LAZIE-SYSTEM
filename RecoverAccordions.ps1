...
if (Test-Path $path) {
    ...
    # Remove any existing notes_accordion or restricted_modal if they exist to prevent duplicates
    if ($content -match 'th:fragment="notes_accordion"') {
        # This shouldn't happen based on our analysis, but just in case
        Write-Output "notes_accordion already exists."
    } else {
        $insertion = @'
...
'@
        $content = $content -replace '</body>', "$insertion`n</body>"
        Set-Content -Path $path -Value $content -Encoding UTF8
        Write-Output "Successfully recovered and injected the fully premium accordions!"
    }
} else {
