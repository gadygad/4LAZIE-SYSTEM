...
public class NotesController {
    ...
    // Renders __PROXY_URL__ via PDF.js instead of a native browser/iframe PDF
    // viewer, since that support is inconsistent across real devices (mobile
    // Safari in particular). PDF.js fetches the raw bytes and paints every
    // page onto its own <canvas>, so the result is identical everywhere.
    private static final String PDF_VIEWER_TEMPLATE = """
            <!DOCTYPE html>
            <html>
            <head>
            <meta charset="UTF-8">
    ...
    public void viewNotePage(@PathVariable("slug") String slug, 
        ...
        if (note.getFileUrl() != null && !note.getFileUrl().isEmpty()) {
            ...
            // Both a raw PDF navigation AND an iframe embedding the browser's
            // native PDF plugin turned out unreliable across real devices —
            // several mobile browsers (mobile Safari especially) either save
            // the file to disk instead of displaying it, or simply render
            // nothing. PDF.js sidesteps browser PDF-viewer support entirely:
            // it downloads the raw bytes and paints every page onto a
            // <canvas> itself, so behavior is identical on every device.
            String safeTitle = note.getTitle() != null
                    ? note.getTitle().replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace("\"", "&quot;")
                    : "Document";
            String viewerHtml = PDF_VIEWER_TEMPLATE
                    .replace("__TITLE__", safeTitle)
                    .replace("__PROXY_URL__", proxyUrl)
                    .replace("__DOWNLOAD_URL__", "/download/" + note.getId());
